...
//                .max((name1, name2) -> name1.length() - name2.length());
    ...
    private static void task10(){
         // min, max, Optional
        // Optional<T> - класс обертка, который может содержать или обьект типа Т или null
        // Optional<User>
        ...
        System.out.println("max.is Empty(): " + max.isEmpty()); // вернет true, если значение отсутствует (там внутри  null) //Java

        if (max.isPresent()) {
            Integer value = max.get(); // возвращает значение, если оно присутствует. Если внутри null - будет исключение (ошибка)
        ...
        //
        Optional.empty(); //
        Optional.of(new Object()); // возвращает Optional.
        Optional.ofNullable(null); // возвращает Optional.
    ...
    private static void task8(){
        ...
        Cat[] cats = new Cat[]{cat, cat1, cat2, cat3, new Cat(null, 10, "red")};
      ...
      //          .filter(c -> c != null ) // оставить только не null
       //         .filter(c -> Objects.nonNull(c)) // оставить только ne null
        ...
        Stream<Cat> catStream = cats.stream()
                .filter(cat -> cat.getWeight() > 4)// фильтруем котиков
