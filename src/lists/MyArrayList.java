...
/*
 Iterable - означает, что все обьекты этого класса можно перебирать (итерировать). Создает всего один метод,
 который возврощает итератор (тот самый обьект - закладку, который позволяет перебирать коллекцию)
// Iterator<T> iterator() -
boolean hasNext
