package сonsultation;


//     **Аутентификация по SSH-протоколу и настройка на GitHub**
//
//     SSH (Secure Shell) — это протокол для безопасного доступа к удаленным серверам. В контексте GitHub,
//     SSH позволяет клонировать репозитории и выполнять другие действия без необходимости вводить имя пользователя и
//     пароль каждый раз.
//
// ### **Публичный и приватный ключи**
//
//             - **Приватный ключ**: хранится на вашем компьютере и должен оставаться конфиденциальным. Он используется
//             для дешифрования сообщений, зашифрованных публичным ключом.
// - **Публичный ключ**: этот ключ вы можете свободно распространять. Он размещается на сервере (например, на GitHub) и
// используется для шифрования сообщений, которые может расшифровать только соответствующий приватный ключ.
//
//             ### **Как работает SSH-аутентификация**
//
//     Когда вы пытаетесь подключиться к серверу, сервер использует ваш публичный ключ для создания зашифрованного сообщения.
//     Ваш компьютер использует приватный ключ для его расшифровки, подтверждая таким образом вашу личность.
//
//             ---
//
...
//
//             4. Когда вас спросят о месте сохранения ключа, нажмите **Enter** для сохранения по умолчанию
...
//
//             4. Когда появится запрос на указание пути сохранения, нажмите **Enter** для сохранения по умолчанию
...
//
//     Теперь, когда вы настроили SSH-аутентификацию с GitHub, при клонировании репозиториев или связывании локальных репозиториев
//     с удаленными вам необходимо использовать **SSH-ссылку** вместо HTTPS. Это обеспечит использование вашего SSH-ключа
//     для аутентификации, что избавит от необходимости вводить логин и пароль при каждой операции.
//
...
//    ```
//
//     Теперь ваш локальный репозиторий будет взаимодействовать с GitHub по SSH-протоколу,
//     используя ваш SSH-ключ для аутентификации.
//
//     Использование SSH-ссылок при работе с GitHub обеспечивает более безопасное и удобное взаимодействие с репозиториями.
//     Это устраняет необходимость вводить учетные данные каждый раз и позволяет использовать созданные вами SSH-ключи
//     для аутентификации.
